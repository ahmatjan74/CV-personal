...
class _PyamidPoolingModule(nn.Module):
    def __init__(self, in_dim, reduction_dim, setting):
        super(_PyamidPoolingModule, self).__init__()
        self.features = []
        for s in setting:
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, pretrained=True, use_aux=True):
        ...
        self.ppm = _PyamidPoolingModule(2048, 512, (1, 2, 3, 6))
    ...
    def forward(self, x):
        ...
        if self.training and self.use_aux:
            return F.upsample(x, x_size[2:], mode='bilinear'), \
                   F.upsample(aux, x_size[2:], mode='bilinear')
        return F.upsample(x, x_size[2:], mode='bilinear')
