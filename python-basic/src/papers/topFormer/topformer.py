    ...
    def __init__(
        ...
        for i, (k, t, c, s) in enumerate(cfgs):
            ...
            layer = InvertedResidual(inp_channel, output_channel, ks=k, stride=s,
                                     expand_ratio=t, norm_cfg=norm_cfg, activations=activation)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, key_dim, num_heads, mlp_ratio=4., attn_ratio=2., drop=0.,
        ...
        self.attn = Attention(dim, key_dim=key_dim, num_heads=num_heads,
                              attn_ratio=attn_ratio, activation=act_layer, norm_cfg=norm_cfg)
...
class InjectionMultiSumCBR(nn.Module):
    ...
    def forward(self, x_l, x_g):
        ...
        # kernel
        global_act = self.global_act(x_g)
        global_act = F.interpolate(self.act(global_act), size=(
            H, W), mode='bilinear', align_corners=False)
        # feat_h
        global_feat = self.global_embedding(x_g)
        global_feat = F.interpolate(global_feat, size=(
...
class FuseBlockSum(nn.Module):
    def __init__(
        ...
        self.fuse1 = ConvModule(inp, oup, kernel_size=1,
                                norm_cfg=self.norm_cfg, act_cfg=None)
        self.fuse2 = ConvModule(inp, oup, kernel_size=1,
                                norm_cfg=self.norm_cfg, act_cfg=None)
    ...
    def forward(self, x_l, x_h):
        ...
        feat_h = F.interpolate(kernel, size=(
            H, W), mode='bilinear', align_corners=False)
        out = inp + feat_h
        return out
...
class FuseBlockMulti(nn.Module):
    ...
    def forward(self, x_l, x_h):
        ...
        sig_act = F.interpolate(self.act(sig_act), size=(
            H, W), mode='bilinear', align_corners=False)
        out = inp * sig_act
        return out
...
class Topformer(BaseModule):
    def __init__(self, cfgs,
        ...
        # token pyramid modules => make different size layers
